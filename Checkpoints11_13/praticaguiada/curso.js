...
// nome do curso (string), nota de aprovação (number), 
...
//  e ter menos faltas que faltas máximas. Se tiver a mesma quantidade, tem que estar 10% acima da nota de aprovação.

// Micro desafio - Passo 6
// Crie um método para o objeto curso que percorra a
// lista de estudantes e retorne um array de booleanos com os resultados se os alunos aprovaram ou não.

// Micro desafio - Passo 7
// Importe o módulo estudantes.js, que contém uma lista de alunos, 
// dentro do arquivo que contém o objeto curso. 
// Substitua o conteúdo da propriedade lista de estudantes, pela lista de estudantes do arquivo estudantes.js 
...
// ... parametro rest - você pode passar vários parametros de uma só vez. 
...
let curso = {
    ...
    faltasMax: 3, 
    listaAlunos: [],
    addAluno: function(...cadastroAluno){
        this.listaAlunos.push(...cadastroAluno)
    },
    aprovarAluno:function(aluno) {
        let mediadoAluno= aluno.calcularMedia()
        if (aluno.quantidadeFaltas < this.faltasMax && mediadoAluno >= this.notaAprovacao) {
            return true; 
        }else if (aluno.quantidadeFaltas == this.faltasMax && mediadoAluno >= this.notaAprovacao*1.1){
            return true;
        }else {
            return false;
        }
    ...
    listar: function() {
        let lista =[];
        this.listaAlunos.forEach(elemento =>{
             lista.push(this.aprovarAluno(elemento))
        })
        return lista
    }
}

// exportanto o objeto curso 
